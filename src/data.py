...
class StrategyQA(BaseDataset):
    ...
    def get_real_prediction(self, pred):
        # 使用正则提取 "the answer is" 后面的内容，同时以句号、换行、Question:和字符串结尾作为截断
        # match = re.search(r"the answer is\s+(\w+)", pred, re.IGNORECASE)
        match = re.search(r"the answer is\s+(.+?)(?:\.|\n|</s>|<\|endoftext\|>|Question:|$)", pred, re.IGNORECASE)
        if match:
...
class WikiMultiHopQA(BaseDataset):
    ...
    def get_real_prediction(self, pred):
        # 匹配 "the answer is" 后的一段文字，直到换行、句点、</s>、<|endoftext|> 或字符串结尾
        # match = re.search(r"the answer is\s+(.+?)(?:</s>|<\|endoftext\|>|\.|\n|$)", pred, re.IGNORECASE)
        match = re.search(r"the answer is\s+(.+?)(?:</s>|<\|endoftext\|>|\.|\n|Question:|$)", pred, re.IGNORECASE)
        if match:
...
class HotpotQA(BaseDataset):
    ...
    def get_real_prediction(self, pred):        
        # 使用正则在原始字符串上查找，不依赖 lower() 的位置
        # match = re.search(r"the answer is\s+([^\n\.]+)", pred, re.IGNORECASE)      
        match = re.search(r"the answer is\s+(.+?)(?:</s>|<\|endoftext\|>|\.|\n|Question:|$)", pred, re.IGNORECASE)  
        if match:
            ans = match.group(1).strip()
            # 可选清洗特殊 token
            ans = ans.replace("</s>", "").replace("<|endoftext|>", "").strip()
            return ans
        else:
            return ""
